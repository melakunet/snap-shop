...
function mockResults(query: string, whitelist: string[]): ShopItem[] {
  const all: ShopItem[] = [
    { price: '$29.99', extracted_price: 29.99, delivery: 'Free delivery', source: 'Amazon.com', link: `https://amazon.com/s?k=${encodeURIComponent(query)}`, thumbnail: '', rating: 4.7, review_count: 12543, title: query, snippet: `Top-rated ${query} with fast Prime delivery. Highly rated by thousands of customers.`, product_id: 'mock_amz_1', mock: true },
    { price: '$34.95', extracted_price: 34.95, delivery: '2-day shipping', source: 'Walmart', link: `https://walmart.com/search?q=${encodeURIComponent(query)}`, thumbnail: '', rating: 4.5, review_count: 3871, title: query, snippet: `Save on ${query} at Walmart. Everyday low prices with free 2-day shipping on eligible orders.`, product_id: 'mock_wmt_2', mock: true },
    { price: '$42.00', extracted_price: 42.00, delivery: 'Free shipping', source: 'Target', link: `https://target.com/s?searchTerm=${encodeURIComponent(query)}`, thumbnail: '', rating: 4.3, review_count: 918, title: query, snippet: `Shop ${query} at Target. Free shipping on orders over $35 or free same-day pickup in store.`, product_id: 'mock_tgt_3', mock: true },
    { price: '$38.50', extracted_price: 38.50, delivery: 'Ships from Nike', source: 'Nike', link: `https://nike.com/search?q=${encodeURIComponent(query)}`, thumbnail: '', rating: 4.8, review_count: 204, title: query, snippet: `Official Nike ${query}. Authentic product direct from Nike with free returns.`, product_id: 'mock_nke_4', mock: true },
    { price: '$31.99', extracted_price: 31.99, delivery: 'Free shipping over $35', source: 'Best Buy', link: `https://bestbuy.com/site/searchpage.jsp?st=${encodeURIComponent(query)}`, thumbnail: '', rating: 4.6, review_count: 1102, title: query, snippet: `Find ${query} at Best Buy with expert advice and price match guarantee.`, product_id: 'mock_bbuy_5', mock: true },
    { price: '$27.50', extracted_price: 27.50, delivery: 'Varies', source: 'eBay', link: `https://ebay.com/sch/i.html?_nkw=${encodeURIComponent(query)}`, thumbnail: '', title: query, mock: true },
    { price: '$44.00', extracted_price: 44.00, delivery: 'Free shipping', source: 'Adidas', link: `https://adidas.com/us/search?q=${encodeURIComponent(query)}`, thumbnail: '', rating: 4.4, review_count: 567, title: query, snippet: `Official Adidas ${query}. Performance and style with free shipping on all orders.`, product_id: 'mock_adi_7', mock: true },
  ]
...
export async function fetchShoppingResults(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[serpapi] SerpAPI ${res.status}: ${body.slice(0, 300)}`)
    ...
    if (data.error) {
      console.error(`[serpapi] SerpAPI error: ${data.error}`)
      return [] // Live mode: return empty list on error
    }
    ...
    console.error('[serpapi] unexpected error:', err)
    return [] // Live mode: return empty list on error
  }
}
